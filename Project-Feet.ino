...
void readSensorsOnly() {
  ...
  float centerx = calibrationValues[0][0];
  float centery = calibrationValues[0][1];
  ...
  if(abs((calibrationValues[2][0]+calibrationValues[1][0])/2-xdistance) > abs((calibrationValues[3][1]+calibrationValues[4][1])/2-ydistance)){
    if (xdistance < centerx) {
      data["key"] = "a";
    } else if (xdistance > centerx) {
      data["key"] = "d";
    }
  } else {
    if (ydistance < centery) {
      data["key"] = "w";
    } else if (ydistance > centery) {
      data["key"] = "s";
    }
  }
